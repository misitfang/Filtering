...
private:
    ...
    int sensor_model_;
    bool floor_removal_;
    double sensor_height_, max_slope_, vertical_thres_;
    ...
    void pointcallback(const pcl::PointCloud<velodyne_pointcloud::PointXYZIR>::ConstPtr &in_cloud_msg);
...
void ring_ground_filter::InitLabelArray(int in_model)
{
...
void ring_ground_filter::InitRadiusTable(int in_model)
{
    double a;
    double b;
    double theta;
...
void ring_ground_filter::InitDepthMap(int in_width)
{
    const int mone = -1;
    index_map_ = cv::Mat_<int>(vertical_res_, in_width, mone);
}
void ring_ground_filter::FilterGround(const pcl::PointCloud<velodyne_pointcloud::PointXYZIR>::ConstPtr &in_cloud_msg,
                                      pcl::PointCloud<velodyne_pointcloud::PointXYZIR> &out_groundless_points,
                                      pcl::PointCloud<velodyne_pointcloud::PointXYZIR> &out_ground_points)
{
...
void ring_ground_filter::pointcallback(const pcl::PointCloud<velodyne_pointcloud::PointXYZIR>::ConstPtr &in_cloud_msg)
{
...
{   
    // node_handle_("~");
    ROS_INFO("Inititalizing");
    node_handle_.param<std::string>("point_topic", point_topic_, "/point_raw");
    ROS_INFO("input point topic:%s", point_topic_.c_str());
    ...
    point_node_sub_ = node_handle_.subscribe(point_topic_, 10000, &ring_ground_filter::pointcallback, this);
    ROS_INFO("debug");
